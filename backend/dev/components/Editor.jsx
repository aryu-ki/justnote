...
export default class Editor extends React.Component {
    ...
    async saveNote(auto = false){
        try {
            let response = await fetch('/save_note', {
                ...
                body: JSON.stringify({note: this.state.note, code: this.state.code})
            })
    ...
    render() {
        return <div className={this.props.className}>
            <TextField className={this.props.className + '-textbox'} onChange={(e) => this.handleChange(e)}  />
            <Button className={this.props.className + '-save'} onClick={() => this.saveNote()} value={"Write."}  />
        </div>
    }
}